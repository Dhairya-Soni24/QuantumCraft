  ...
  return (
    <header className="flex h-12 shrink-0 items-center justify-between border-b border-[#3c494c] bg-[#0c1324] px-5">
      ...
      <div className="flex items-center gap-4">
        <span className="text-lg text-[#bbc9cd]">☾</span>
...
function TutorPanel() { return <aside className="hidden w-[310px] shrink-0 flex-col border-l border-[#3c494c] bg-[#151b2d] xl:flex"><div className="flex h-10 items-center justify-between border-b border-[#3c494c] px-3 text-[11px] uppercase tracking-wider text-[#dce1fb]">◉ AI Tutor <span>•••</span></div><div className="relative flex-1 p-4"><div id="ai-tutor-chat" aria-label="AI tutor chat mount point" className="absolute inset-0"/><div className="mb-4 flex gap-2"><Bot size={24} className="shrink-0 text-[#c084fc]"/><div className="bg-[#2e3447] p-3 text-sm leading-6 text-[#dce1fb]">I noticed you created a Bell State. This is an entangled state where the two qubits are perfectly correlated.<br/><br/>Would you like to learn how to implement quantum teleportation using this state?</div></div><button className="ml-9 border border-[#22d3ee]/40 bg-[#22d3ee]/10 px-3 py-2 text-left text-sm text-[#dce1fb]">Yes, show me the circuit diagram.</button></div><div className="relative h-48 border-t border-[#3c494c] p-3"><div id="plotly-output" aria-label="Bloch sphere and Plotly mount point" className="absolute inset-0"/><h2 className="mb-3 text-[11px] uppercase tracking-wider text-[#dce1fb]">▥ Simulation Output</h2><div className="h-28 border border-[#3c494c] bg-[#020617] p-4 text-center font-mono text-xs text-[#bbc9cd]">Probabilities (Shots: 1024)<div className="mt-8 text-[#8aebff]">──────　──────<br/> |00⟩　 |11⟩</div></div></div></aside>; }

export default function Home() { return <div className="flex h-screen min-h-[680px] flex-col overflow-hidden bg-[#020617] font-sans text-[#dce1fb]"><TopNav/><main className="flex min-h-0 flex-1"><ActivityBar/><GatePanel/><div className="flex min-w-0 flex-1 flex-col"><CircuitPanel/><CodePanel/></div><TutorPanel/></main><footer className="flex h-6 shrink-0 items-center gap-5 border-t border-[#3c494c] bg-[#00363e] px-3 font-mono text-[11px] text-[#8aebff]"><span>● Simulator: Aer</span><span>State: Idle</span><span className="ml-auto text-[#bbc9cd]">Ln 8, Col 21　 UTF-8　 Python 3.10</span></footer></div>; }
